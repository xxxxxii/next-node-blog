...
const Tag: FC<propsType> = (props) => {
  // const { data: dataList, isLoading } = useFetch(
  //   () => {
  ...
  let [data, setData] = useState<any>([]); //选中的tag列表ID
          ...
          return (
            <Button key={item} className="mr-1">
              ...
              <CloseOutlined
                ...
                onClick={() =>
                  setData((_data: any[]) =>
                    _data.filter((_item: any) => _item != item)
                  )
                }
              />
            </Button>
          ...
          <div className="flex w-96 justify-start rounded-md bg-white p-3 shadow-xl">
            {isLoading ? (
              ...
              <div className="w-full">
                ...
                <div className="flex w-full flex-wrap">
                  {dataList
                    ?.find((i: { id: number }) => i.id == activeType)
                    ?.children?.map(
                      (
                        item: {
