...
import type { Directive, DirectiveBinding, StyleValue, VNode, WatchStopHandle } from 'vue';
export const name = 'z-skeleton',
    preCls = `${name}`,
...
export const patchClass = (el: HTMLElement, clazz, oldClazz) => {
    ...
    const delCls = oldClazz.filter((e) => !clazz.includes(e)),
        addCls = clazz;
    delCls.forEach((e) => e && el.classList.remove(e));
    addCls.forEach((e) => e && el.classList.add(e));
};

export const getLoading = (binding: Props['binding']) =>
    typeof binding.value === 'object' ? binding.value.loading : binding.value;
...
export const fliterClass = (cls: string) => {
    return cls
        .split(' ')
        .filter((cls) => cls.includes(preCls))
        .join(' ');
};
...
const setup = (el: Props['el'], binding: Props['binding'], vnode: Props['vnode']) => {
    ...
    const unwatch1 = watchPostEffect(() => {
        ...
        if (getLoading(binding)) {
            ...
            // @ts-ignore
            el.style.cssText = stringifyStyle(
                normalizeStyle([el.style.cssText, binding.value.loadingStyle])
            );
        } else {
...
const Skeleton: Directive = {
    created(el, binding, vnode) {
        console.log(el, binding, vnode, 'binding, vnode');
