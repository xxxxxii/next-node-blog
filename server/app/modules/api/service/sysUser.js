...
class SysUserService {
  ...
  static async find(username, password) {
    try {
      const res = await knex(`${SysUserService.model}`)
        .where({
          username: `${username}`,
          password: `${password}`,
        })
  ...
  static async delete(id) {
    try {
      const result = await knex(SysUserService.model)
        .where("id", "=", id)
        .del();
      return result ? "success" : "fail";
    } catch (err) {
  ...
  static async list(cur = 1, pageSize = 10) {
    try {
      // 查询个数
      const total = await knex(SysUserService.model).count("id", {
        as: "count",
      });
